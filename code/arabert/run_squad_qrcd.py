...
'''
Main updates (on Feb. 05, 2021):
- writing the start token position (orig_start_index), and the end token position 
  (orig_end_index) of each predicted answer to the nbest_predictions.json file.
- random_seed can now be passed as a parameter
'''
...
def write_predictions(
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        ## by rana
        _NbestPrediction = collections.namedtuple(  # pylint: disable=invalid-name
            "NbestPrediction", ["text", "start_logit", "end_logit", "orig_start_index", "orig_end_index"]
        )
